...
def get_embedding(text: str) -> List[float]:
    ...
    try:
        ...
        logger.info(f"Using OpenAI API key: {OPENAI_API_KEY[:5]}...{OPENAI_API_KEY[-4:]}")
        ...
        # Trim to 1024 dimensions to match Pinecone index
        if len(embedding) > 1024:
            logger.info(f"Trimming embedding from {len(embedding)} to 1024 dimensions")
